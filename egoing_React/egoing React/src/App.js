    ...
    return (
      <article>
        <form onSubmit={function(ev){
          ...
          <p>
            <input type="text" placeholder="title" name="title" value={this.state.title} onChange={this.changeFormHandler.bind(this)}></input>
          </p>
          <p>
            <textarea placeholder="desc" name="desc" onChange={this.changeFormHandler.bind(this)}></textarea>
          </p>
          <p>
...
class App extends Component {
  ...
  getControlComponent() {
    return [
      ...
      <input key="3" type="button" href="/delete" onClick={function(){
        var newContents = this.state.contents.filter(function(el){ // 배열에 담겨있는 숫자만큼 그 순번에 해당하는 el에 해당하는 배열에 전달
  ...
  render() {
    return (
      <div className="App">
        <Subject title="WEB" sub="Hello WEB" onClick={function(){
          this.setState({
            mode:'welcome'
          });
        }.bind(this)}></Subject>
        <TOC data={this.state.contents} onSelect={function(id){
          this.setState({
